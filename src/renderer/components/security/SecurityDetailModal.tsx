...
export function SecurityDetailModal({ title, icon, severity, children, actions, onClose }: SecurityDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="pcd-modal w-full max-w-lg p-5 shadow-2xl max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex justify-end gap-2 flex-wrap">
          <button onClick={onClose} className="px-3 py-1.5 rounded-md text-xs pcd-button">
            Close
          </button>
          {actions}
        </div>
