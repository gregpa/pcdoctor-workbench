<#
.SYNOPSIS
    Clears OneDrive logs and setup cache directories.
#>
param(
    [switch]$DryRun,
    [switch]$JsonOutput
...
$result = @{ success = $true; duration_ms = $sw.ElapsedMilliseconds; bytes_freed = $freed; message = "Cleared OneDrive cache ($([math]::Round($freed/1MB,1)) MB)" }
