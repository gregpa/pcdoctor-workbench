...
$exit = $LASTEXITCODE
$result = @{ success = ($exit -eq 0); duration_ms = $sw.ElapsedMilliseconds; exit_code = $exit; output = ($output.Trim() -split "`r?`n" | Select-Object -Last 10) -join "`n"; message = 'DISM RestoreHealth completed' }
